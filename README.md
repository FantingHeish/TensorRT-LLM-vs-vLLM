...
### 🔸 1. Async Decode Thread（推論放入背景 Thread）

推論 (model.generate) 被丟到背景 Thread 執行：
Background Thread
執行 Prefill（重計算）
...
👉 推論與輸出分離，使第一個token 更快送出（降低 TTFT）。

### 🔸 2. Streaming Pipeline：token 一生成就送到 client
🎯 Streaming 流程
背景 Thread 執行 model.generate()
每生成一個 token → push 到 TextIteratorStreamer queue
FastAPI SSE handler 逐 token 傳輸：
